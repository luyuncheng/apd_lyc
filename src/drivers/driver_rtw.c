...
static int rtl871x_set_iface_flags(void *priv, int dev_up)
{
    ...
    printf("lyc_rtl871x_set_iface_flags\n");
...
{
    struct iwreq iwr;
    printf("lyc_rtl871x_hostapd_ioctl\n");
                                            ...
                                            char *data, int len)
{
    ...
    printf("lyc_rtl871x_wireless_event_wireless\n");
    pos = data;
    end = data + len;
    printf("lyc_data:\n\n\n");
//    int zz=0;
...
//     printf("lyc_data:\n\n\n");
    ...
    while (pos + IW_EV_LCP_LEN <= end) {
        ...
        //把data从pos位置开始IW_EV_LCP_LEN个长度到iwe_buf
        memcpy(&iwe_buf, pos, IW_EV_LCP_LEN);
        wpa_printf(MSG_MSGDUMP, "Wireless event: cmd=0x%x len=%d",
                   iwe->cmd, iwe->len);
        printf("LYC_(rtl871x_wireless_event_wireless)---Wireless event: cmd=0x%x len=%d\n",
                   iwe->cmd, iwe->len);
        ...
        //lyc debug iwe->cmd
        if(iwe->cmd== IWEVEXPIRED){printf("IWEVEXPIRED:\n");}
        else
        if(iwe->cmd== IWEVREGISTERED){printf("IWEVREGISTERED:\n");}
        else
        if(iwe->cmd== IWEVCUSTOM){printf("IWEVCUSTOM:\n");}
        else
            printf("no cmd\n");
        //lyc debug iwe->cmd ed
        switch (iwe->cmd) {
            case IWEVEXPIRED:
                                               ...
                                               struct ifinfomsg *ifi, u8 *buf, size_t len)
{
    ...
    printf("lyc_rtl871x_wireless_event_rtm_newlink:\n");
    if (ifi->ifi_index != drv->ifindex)
        return;

    printf("*****newlink frame begin******\n");

    //wpa_hexdump(MSG_DEBUG, "ifi: ", (u8 *) ifi, sizeof(struct if     infomsg));
    attrlen = len;
    ...
    printf("lyc_rtw.c_attrlen=%d\n", attrlen);   
    wpa_hexdump(MSG_DEBUG, "lyc_attr1: ", (u8 *) attr, sizeof(struct      rtattr));



    rta_len = RTA_ALIGN(sizeof(struct rtattr));
    wpa_hexdump(MSG_DEBUG, "lyc_attr2: ", (u8 *)attr,rta_len);

    rta_len = RTA_ALIGN(sizeof(struct rtattr));
    while (RTA_OK(attr, attrlen)) {
        if (attr->rta_type == IFLA_WIRELESS) {
      //      wpa_printf(MSG_DEBUG,"lyc_rta_type=%02x\n",attr->tra_type);
            rtl871x_wireless_event_wireless(
                                            drv, ((char *) attr) + rta_len,
                                            attr->rta_len - rta_len);
        }
        attr = RTA_NEXT(attr, attrlen);
        wpa_hexdump(MSG_DEBUG,"lyc_attr3: ",(u8 *) attr,sizeof(struct rtattr));
    }
    
    printf("*****newlink frame end********\n");
}
                                               ...
                                               struct nlmsghdr *h, int len)
{
    ...
    struct rtattr * attr;
    printf("lyc_rtl871x_wireless_event_rtm_newlink2\n");
...
static int rtl871x_wireless_event_init(struct rtl871x_driver_data *drv)
{
    ...
    printf("lyc_rtl871x_wireless_event_init\n");
    ...
    printf("lyc_wireless_event_init:next is netlink init(cfg)\n");
...
static void rtl871x_handle_read(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    printf("lyc_rtl871x_handle_read\n");
    /*lyc begin_driver_rtl871x_handle_read*/
    //struct sta_info *sta;
    //sta = ap_get_sta(drv->hapd,src_addr);
    //printf("lyc_driver_rtw.c,sta_mac： "MACSTR" \n",MAC2STR((sta->addr)));
    
    /*lyc end_driver_rtl871x_handle_read*/
                                  ...
                                  int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    printf("lyc_rtl871x_send_eapol_ops\n");
    struct rtl871x_driver_data *drv = priv;
...
static void rtl871x_receive_mgnt(struct rtl871x_driver_data *drv , const u8 *buf, size_t len)
{
    ...
    printf("lyc_rtl871x_receive_mgnt\n");
    ...
    printf("lyc_rtl871x_receive_mgnt, " MACSTR " is our address\n", MAC2STR(hapd->own_addr));
    ...
    {
        printf("lyc_rtl871x_MGNT Frame - PROBE_RESP Frame\n");
    }
#endif
    ...
    switch (type) {
        case WLAN_FC_TYPE_MGMT:
            ...
            {
                printf("lyc：rtl871x_receive_mgnt, type=0x%x, stype=0x%x\n", type, stype);
            }
...
{
   //回调函数 收到原始帧 
    printf("lyc_rtl871x_recvive_mgmt_frame\n");
    struct rtl871x_driver_data *drv = ctx;
...
static int rtl871x_mgnt_sock_init(struct rtl871x_driver_data *drv, const char *name)
{
    ...
    printf("lyc_rtl871x_mgnt_sock_init\n");
    int sock;
    struct ifreq ifr;
   ...
   //建立原始套接字 
    ...
    ////把原始套接字的描述符和回调函数rtl871x_recvive_mgmt_frame注册到event loop  
                                       ...
                                       int ok)
{
    ...
    printf("lyc_rtl871x_handle_tx_callback\n");
#if 0
                                       ...
                                       int flags)
{
    ...
    printf("lyc_rtl871x_send_mgmt_frame_ops\n");
    //printf("%s\n", __func__);
...
#define MAX_NUM_CHANNEL_5G (24)
    ...
    printf("lyc_rtl871x_hw_modes*rtl871x_get_hw_feature_data_ops\n");
    *num_modes = 3;
    *flags = 0;
...
static int rtl871x_sta_remove_ops(void *priv, const u8 *addr)
{
    ...
    printf("lyc_rtl871x_sta_remove_ops\n");
    struct rtl871x_driver_data *drv = priv;
    struct ieee_param param;
...
static int rtl871x_set_hidden_ssid_ops(const char *iface, void *priv, u8 value)
{
    ...
    printf("lyc_rtl871x_set_hidden_ssid_ops\n");
    printf("%s\n", __func__);
                                  ...
                                  int beacon_int)
{
    ...
    printf("lyc_rtl871x_set_beacon_ops\n");
    int ret;
    size_t sz;
    ieee_param *pparam;
    struct rtl871x_driver_data *drv = priv;
                               ...
                               size_t seq_len, const u8 *key, size_t key_len)
{
    ...
    printf("lyc_rtl871x_set_key_ops\n");
    ieee_param *param;
    u8 *buf;
    char *alg_str;
    size_t blen;
...
static int rtl871x_set_wps_assoc_resp_ie(struct rtl871x_driver_data *drv, const void *ie, size_t len)
{
    ...
    printf("lyc_rtl871x_set_wps_assos_resp_ie\n");
    printf("%s\n", __func__);
...
static int rtl871x_set_wps_beacon_ie(struct rtl871x_driver_data *drv, const void *ie, size_t len)
{
    ...
    printf("lyc_rtl871x_set_wps_beacon_ie\n");
    printf("%s\n", __func__);
...
static int rtl871x_set_wps_probe_resp_ie(struct rtl871x_driver_data *drv, const void *ie, size_t len)
{
    ...
    printf("lyc_rtl871x_set_wps_probe_resp_ie\n");
                                 ...
                                 const struct wpabuf *proberesp, const struct wpabuf *assocresp)
{
    ...
    printf("lyc_rtl871x_set_ap_wps_ie\n");
    //if(proberesp){
    //    printf("\n\ntest：lyc_ap_wps_ie:\n\n\n");
    //    u8 *itss=wpabuf_head_u8(proberesp);
    //    printf("wpabuf_len:%d",wpabuf_len(proberesp));
        //while(itss){
        //    printf("0x%x",itss);
        //    itss++;
        //}
...
static int rtl871x_sta_flush_ops(void *priv)
{
    ...
    printf("lyc_rtl871x_sta_flush_ops\n");
    memset(&param, 0, sizeof(param));
...
static void *rtl871x_driver_init_ops(struct hostapd_data *hapd, struct wpa_init_params *params)
{
    ...
    printf("lyc_rtl871x_driver_init_ops\n");
...
static void rtl871x_driver_deinit_ops(void *priv)
{
    ...
    printf("lyc_rtl871x_driver_deinit_ops\n");
